...
const logReprint = (row: number, msg: string) => {
...
class Renderer {
  render(state: GameState) {
    ...
    for (const player of state.players) {
      ...
      board[y][x] = colorstring(defaultColors[player.id], player.name.charAt(0));
    }

    board
      .map(c => c.join(''))
      .forEach((line, lineNumber) => logReprint(lineNumber, line));
  }
}
